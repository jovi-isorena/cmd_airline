...
<div class="container pt-5 full-h">
    ...
    <?php endif;?>
    <form method="post" action="<?php echo URLROOT . '/flightExtras/addBaggage/' . $data['flightNo']; ?>">
        ...
        <div class="form-group">
            <label for="extra">Baggage Package</label>
            <span style="color: red;">* <?php echo $data['extraError'];?></span>
            <select name="extra" id="extra" class="form-control custom-select">
                <option value='' <?php echo empty($data['extra'])?'selected':'';?>>Select a Baggage Package</option>
                <?php foreach($data['baggages'] as $baggage):?>
                    <option value="<?php echo $baggage->id;?>" <?php echo $data['extraId']==$baggage->id?'selected':'';?>><?php echo $baggage->name . ' - $' . $baggage->price;?></option>
                <?php endforeach;?>
            </select>
        </div>
